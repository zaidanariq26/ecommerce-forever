...
const Dashboard = ({ token }) => {
		...
		const fetchData = async () => {
			try {
				const [ordersRes, productsRes, activityRes] = await Promise.all([
					...
					axios.get(BACKEND_URL + "/api/activity/list?limit=10", {
						headers: { token },
					}),
				]);
	...
	const lowStockTotalPages = Math.ceil(
		lowStockProducts.length / LOW_STOCK_PER_PAGE,
	);
	const lowStockPageItems = lowStockProducts.slice(
			...
			{lowStockProducts.length > 0 && (
				<div className="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/30 p-6">
					<div className="mb-4 flex items-center gap-2">
						...
						/>
						<h2 className="text-lg font-medium text-amber-800 dark:text-amber-300">
							Low Stock Alerts
						</h2>
					...
					{lowStockTotalPages > 1 && (
						<div className="mt-4 flex items-center justify-center gap-2">
							<button
								...
								className="cursor-pointer rounded-lg border border-amber-300 dark:border-amber-700 px-3 py-1 text-xs text-amber-700 dark:text-amber-300 disabled:cursor-not-allowed disabled:opacity-40 hover:bg-amber-100 dark:hover:bg-amber-900/30 transition-colors"
							>
								Prev
							</button>
...
const StatCard = ({ icon, label, value, sub, color }) => (
	<div className="flex items-center gap-4 overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 sm:p-5">
		...
		<div className="min-w-0">
			...
			<p className="truncate text-xl font-semibold text-gray-800 dark:text-white">
				{value}
			</p>
			{sub && (
				<p className="truncate text-xs text-gray-400 dark:text-gray-500">
